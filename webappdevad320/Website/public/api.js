...
async function fetchDogBreeds() {
  try {
    ...
    // Logs an error message if there's an issue with fetching or processing data
...
export async function updateDog(i = 0) {
  try {
    ...
    const dogLife =
      dogBreeds[i].lifeSpan.min + " - " + dogBreeds[i].lifeSpan.max;
    const dogWeightM =
      dogBreeds[i].maleWeight.min + " - " + dogBreeds[i].maleWeight.max;
    const dogWeightF =
      dogBreeds[i].femaleWeight.min + " - " + dogBreeds[i].femaleWeight.max;
